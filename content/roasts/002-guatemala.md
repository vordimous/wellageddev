---
title: 002 — Guatemala Huehuetenango Unión Cantinil
date: 2026-06-13T00:00:00.000Z
origin: 'Guatemala — Huehuetenango, Unión Cantinil'
process: null
target_level: Medium
tags:
  - roast
  - guatemala
  - huehuetenango
batch_size_g: 454
profile: P5
weight_setting: 1 lb
green_weight_g: 454
ambient_f: 91
ambient_rh: 44
bean_notes: |
  **Note — wrong model:** The playbook for this roast was built for the Behmor 1600 Plus. The machine used was the Behmor 2000 AB Plus. Roast data is as logged; the playbook is a historical record, not a recommendation.

  - Huehue is high-grown and dense; expect a slightly later FC than the Brazil at the same heat setting.
  - Flavor goal beyond the level-default: cocoa with citrus brightness and a clean finish — don't bake the acidity flat.
playbook: |
  Ambient ~91°F, drum already hot from Brazil — no preheat needed, skip step 2 from roast 001.

  1. Let drum cool 7 min with door open. Empty chaff tray.
  2. Weigh 227 g green into drum.
  3. Insert drum. Select **P5 / ½ lb**, press START. Start phone timer.
  4. Chopstick in door. Expect FC sooner than Brazil — residual drum heat speeds things up.
  5. **75% shutoff — critical:** press START when the display counts down 0:30 or it auto-cools.
  6. **Listen for FC** — likely 9–11 min today. Mark time-to-FC.
  7. **Drop trigger:** ~30s into development, before any second-crack snaps. Hit **C**.
  8. Cool cycle ~10 min.
  9. Weigh roasted.
  10. **Before Nicaragua: cool drum 7 min with door open.**
time_to_fc: '10:10'
color_at_fc: Pale brown
smell_at_fc: 'Roasted corn '
total_time: '18:00'
why_dropped: null
anything_weird: null
roasted_weight_g: 393
tasting_notes: null
rating: null
next_time: null
roaster: Behmor 1600 Plus
---


